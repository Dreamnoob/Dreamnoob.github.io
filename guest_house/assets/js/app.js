...
document.addEventListener('DOMContentLoaded', () => {
	    ...
	    const promoformInputs = document.querySelectorAll('.promoform__input');
	    const calendars = document.querySelectorAll('.calendar');
	    const calendarBottoms = document.querySelectorAll('.calendar__bottom');
	    const calendarTitles = document.querySelectorAll('.calendar__month');
	    const arrowsLeft = document.querySelectorAll('.calendar__arrow--left');
	    const arrowsRight = document.querySelectorAll('.calendar__arrow--right');
	    ...
	    for (let i = 0; i < promoformInputs.length; i++) {
	        nav[i] = 0;
	    }
	    ...
	    function showCalendar(j) {
	        calendars[j].classList.add('active');
	    }
	    ...
	    function load(j) {
	        ...
	        if (nav[j] !== 0) {
	            dt.setMonth(new Date().getMonth() + nav[j]);
	        }
	        ...
	        calendarTitles[j].innerHTML = `${months[month]}`;
	
	        calendarBottoms[j].innerHTML = '';
	        ...
	        for (let i = 1; i <= daysInMonth; i++) {
	            ...
	            selectedDate(daySquare, j);
	
	            calendarBottoms[j].appendChild(daySquare);
	        }
	    }
	    ...
	    try {
	        promoform.addEventListener('click', (e) => {
	            ...
	            if (target && target.classList.contains('promoform__input')) {
	                promoformInputs.forEach((item, i) => {
	                    if (target == item) {
	                        if (calendars[i].classList.contains('active')) {
	    ...
	    function selectedDate(item, j) {
	        item.addEventListener('click', () => {
	            let num = item.innerText;
	
	            let result = months.findIndex(item => {
	                return item == calendarTitles[j].innerText;
	            });
	
	            setSelectedDate(j, getCurrentDay(num), getCurrentMonth(result), year);
	
	            calendars[j].classList.remove('active');
	        });
	    }
	
	    function setSelectedDate(j, day, month, year) {
	        if (j == null) {
	            promoformInputs.forEach(item => {
	                item.innerHTML = `${day}-${month}-${year}`;
	            });
	        } else {
	            promoformInputs[j].innerHTML = `${day}-${month}-${year}`;
	        }
	    }
	    ...
	    setSelectedDate(null, getCurrentDay(day), getCurrentMonth(month), year);
	    initButtons();
	const promoformSelect = document.querySelector('.promoform__select');
	const select = document.querySelector('.select');
	const selectItems = document.querySelectorAll('.select__item');
	const selectNums = document.querySelectorAll('.select__num');
	
	try {
	    promoformSelect.addEventListener('click', () => {
	    ...
	    select.addEventListener('click', (e) => {
	        ...
	        if (target && target.classList.contains('select__item')) {
	            selectItems.forEach((item, i) => {
	                if (target == item) {
	                    promoformSelect.innerHTML = selectNums[i].innerText;
	                    select.classList.remove('active');
	                }
	            });
	        }
	...
	const swiper = new Swiper('.reviews-slider__inner', {
	...
	function toggleNavMenu() {
	    ...
	    function checkWindowWidth() {
	        ...
	        if (windowWidth >= 951) {
	            nav.style.maxHeight = "";
	        } else if (windowWidth <= 950 && nav.classList.contains('active')) {
	...
	function toggleRoomInfo() {
	    ...
	    const roomsButtons = document.querySelectorAll('.rooms__buttons');
	    const btnsMobile = document.querySelectorAll('.rooms__btn-mobile');
	    ...
	    function checkWindowWidth() {
	        ...
	        roomsButtons.forEach(item => {
	            if (windowWidth >= 565) {
	                item.style.maxHeight = "";
	            } else if (windowWidth <= 564 && item.classList.contains('active')) {
	    ...
	    try {
	        roomsList.addEventListener('click', (e) => {
	            ...
	            if (target && target.classList.contains('rooms__btn-mobile')) {
	                btnsMobile.forEach((item, i) => {
	                    if (target == item) {
	                        roomsButtons[i].classList.toggle('active');
	                        btnsMobile[i].classList.toggle('active');
	                        checkClassActive(i);
	                    }
	                }); 
	            }       
	    ...
	    function checkClassActive(j) {
	        if (roomsButtons[j].classList.contains('active')) {
	            roomsButtons[j].style.maxHeight = roomsButtons[j].scrollHeight + "px";
	        } else {
	            roomsButtons[j].style.maxHeight = 0;
	        }
	    }
	    window.addEventListener('resize', checkWindowWidth);
	    checkWindowWidth();
